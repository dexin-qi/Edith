...
#include "modules/vehicle/proto/chassis.pb.h"
...
using edith::vehicle::Chassis;
using edith::localization::LocalizationEstimate;

class VehicleStateProviderTest : public ::testing::Test {
 public:
  virtual void SetUp() {
    std::string localization_file =
        "/edith/modules/localization/testdata/3_localization_result_1.pb.txt";
    CHECK(cyber::common::GetProtoFromFile(localization_file, &localization_));
    chassis_.set_speed_mps(3.0);
    chassis_.set_gear_location(vehicle::Chassis::GEAR_DRIVE);
...
TEST_F(VehicleStateProviderTest, Accessors) {
  ...
  EXPECT_DOUBLE_EQ(vehicle_state_provider->gear(), vehicle::Chassis::GEAR_DRIVE);
}
