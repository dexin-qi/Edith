...
std::shared_ptr<edith::cyber::Writer<LatencyRecordMap>>
LatencyRecorder::CreateWriter() {
  ...
  if (node_ == nullptr) {
    ...
    node_ = edith::cyber::CreateNode(absl::StrCat(
...
void LatencyRecorder::PublishLatencyRecords(
    const std::shared_ptr<edith::cyber::Writer<LatencyRecordMap>>& writer) {
  records_->set_module_name(module_name_);
  edith::common::util::FillHeader("LatencyRecorderMap", records_.get());
