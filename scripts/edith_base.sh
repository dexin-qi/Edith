...
EDITH_ROOT_DIR="$( cd "$( dirname "${BASH_SOURCE[0]}" )/.." && pwd )"
...
function check_in_docker() {
  if [ -f /.dockerenv ]; then
    EDITH_IN_DOCKER=true
  else
    EDITH_IN_DOCKER=false
  fi
  export EDITH_IN_DOCKER
}
...
function start_prof_customized_path() {
  ...
  LOG="${EDITH_ROOT_DIR}/data/log/${MODULE}.out"
  is_stopped_customized_path "${MODULE_PATH}" "${MODULE}"
  if [ $? -eq 1 ]; then
    PROF_FILE="/tmp/$MODULE.prof"
    rm -rf $PROF_FILE
    BINARY=${EDITH_BIN_PREFIX}/modules/${MODULE_PATH}/${MODULE}
    eval "CPUPROFILE=$PROF_FILE $BINARY \
        --flagfile=modules/${MODULE_PATH}/conf/${MODULE}.conf \
        --log_dir=${EDITH_ROOT_DIR}/data/log $@ </dev/null >${LOG} 2>&1 &"
    sleep 0.5
    is_stopped_customized_path "${MODULE_PATH}" "${MODULE}"
    if [ $? -eq 0 ]; then
...
function start_gdb_customized_path() {
  ...
  eval "gdb --args ${EDITH_BIN_PREFIX}/modules/${MODULE_PATH}/${MODULE} \
      --flagfile=modules/${MODULE_PATH}/conf/${MODULE}.conf \
      --log_dir=${EDITH_ROOT_DIR}/data/log $@"
}
...
function help() {
cat <<EOF
Invoke ". scripts/edith_base.sh" within docker to add the following commands to the environment:
...
if [ $EDITH_IN_DOCKER = "true" ]; then
  ...
  create_data_dir
  set_lib_path $1
  if [ -z $EDITH_BASE_SOURCED ]; then
    determine_bin_prefix
    export EDITH_BASE_SOURCED=1
  fi
fi
