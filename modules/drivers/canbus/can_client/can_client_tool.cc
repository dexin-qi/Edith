...
int main(int32_t argc, char **argv) {
  ...
  using edith::drivers::canbus::CanAgent;
  using edith::drivers::canbus::CANCardParameter;
  using edith::drivers::canbus::CanClient;
  using edith::drivers::canbus::CanClientFactory;
  using edith::drivers::canbus::TestCanParam;
  ...
  if (!edith::cyber::common::GetProtoFromFile(FLAGS_can_client_conf_file_a,
                                               &can_client_conf_a)) {
  ...
  if (!FLAGS_only_one_send) {
    if (!edith::cyber::common::GetProtoFromFile(FLAGS_can_client_conf_file_b,
                                                 &can_client_conf_b)) {
      AERROR << "Unable to load canbus conf file: "
