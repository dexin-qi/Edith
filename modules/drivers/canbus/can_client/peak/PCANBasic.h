...
#define PCAN_ERROR_BUSWARNING \
  PCAN_ERROR_BUSHEAVY                   // Bus error: an error counter reached the 'warning' limit
...
#define PCAN_ERROR_ANYBUSERR                                                               \
  (PCAN_ERROR_BUSWARNING | PCAN_ERROR_BUSLIGHT | PCAN_ERROR_BUSHEAVY | PCAN_ERROR_BUSOFF | \
   PCAN_ERROR_BUSPASSIVE)              // Mask for all bus errors
...
#define PCAN_ERROR_REGTEST \
  0x00100U  // Test of the CAN controller hardware registers failed (no hardware found)
...
#define PCAN_ERROR_CAUTION \
  0x2000000U  // An operation was successfully carried out, however, irregularities were registered
#define PCAN_ERROR_INITIALIZE \
  0x4000000U  // Channel is not initialized [Value was changed from 0x40000 to 0x4000000]
#define PCAN_ERROR_ILLOPERATION \
  0x8000000U  // Invalid operation [Value was changed from 0x80000 to 0x8000000]
...
#define PCAN_CHANNEL_IDENTIFYING \
  0x15U  // Physical identification of a USB based PCAN-Channel by blinking its associated LED
#define PCAN_CHANNEL_FEATURES 0x16U  // Capabilities of a PCAN device (FEATURE_***)
#define PCAN_BITRATE_ADAPTING \
  0x17U                          // Using of an existing bit rate (PCAN-View connected to a channel)
...
#define PCAN_ACCEPTANCE_FILTER_11BIT \
  0x22U  // Filter over code and mask patterns for 11-Bit messages
#define PCAN_ACCEPTANCE_FILTER_29BIT \
  0x23U  // Filter over code and mask patterns for 29-Bit messages
#define PCAN_IO_DIGITAL_CONFIGURATION \
  0x24U  // Output mode of 32 digital I/O pin of a PCAN-USB Chip. 1: Output-Active 0 : Output
         // Inactive
#define PCAN_IO_DIGITAL_VALUE 0x25U  // Value assigned to a 32 digital I/O pins of a PCAN-USB Chip
#define PCAN_IO_DIGITAL_SET \
  0x26U  // Value assigned to a 32 digital I/O pins of a PCAN-USB Chip - Multiple digital I/O pins
         // to 1 = High
...
#define PCAN_FIRMWARE_VERSION \
  0x29U  // Get the version of the firmware used by the device associated with a PCAN-Channel
...
#define PCAN_FILTER_CUSTOM \
  0x02U  // The PCAN filter is custom configured. Only registered messages will be received
#define PCAN_CHANNEL_UNAVAILABLE \
  0x00U  // The PCAN-Channel handle is illegal, or its associated hardware is not available
#define PCAN_CHANNEL_AVAILABLE \
  0x01U  // The PCAN-Channel handle is available to be connected (Plug&Play Hardware: it means
         // furthermore that the hardware is plugged-in)
...
#define PCAN_CHANNEL_PCANVIEW \
  (PCAN_CHANNEL_AVAILABLE | PCAN_CHANNEL_OCCUPIED)  // The PCAN-Channel handle is already being used
...
#define LOG_FUNCTION_ALL \
  0xFFFFU  // Logs all possible information within the PCAN-Basic API functions

#define TRACE_FILE_SINGLE 0x00U  // A single file is written until it size reaches PAN_TRACE_SIZE
#define TRACE_FILE_SEGMENTED \
  0x01U  // Traced data is distributed in several files with size PAN_TRACE_SIZE
...
#define FEATURE_DELAY_CAPABLE \
  0x02U  // Device supports a delay between sending frames (FPGA based USB devices)
#define FEATURE_IO_CAPABLE \
  0x04U  // Device supports I/O functionality for electronic circuits (USB-Chip devices)
...
#define PCAN_MESSAGE_BRS \
  0x08U  // The PCAN message represents a FD bit rate switch (CAN data at a higher bit rate)
#define PCAN_MESSAGE_ESI \
  0x10U  // The PCAN message represents a FD error state indicator(CAN FD transmitter was error
         // active)
...
//
// Example:
//    f_clock = 80000000,nom_brp = 10,nom_tseg = 5,nom_tseg2 = 2,nom_sjw = 1,data_brp = 4,data_tseg1
//    = 7,data_tseg2 = 2,data_sjw = 1
//
...
TPCANStatus __stdcall CAN_Initialize(TPCANHandle Channel, TPCANBaudrate Btr0Btr1,
                                     TPCANType HwType _DEF_ARG, DWORD IOPort _DEF_ARG,
...
///</remarks>
/// <example>f_clock = 80000000,nom_brp = 10,nom_tseg = 5,nom_tseg2 = 2,nom_sjw = 1,data_brp =
/// 4,data_tseg1 = 7,data_tseg2 = 2,data_sjw = 1</example>
/// <returns>"A TPCANStatus error code"</returns>
...
/// <returns>"A TPCANStatus error code"</returns>
TPCANStatus __stdcall CAN_Read(TPCANHandle Channel, TPCANMsg* MessageBuffer,
...
/// should be passed as NULL"</param>
/// <returns>"A TPCANStatus error code"</returns>
TPCANStatus __stdcall CAN_ReadFD(TPCANHandle Channel, TPCANMsgFD* MessageBuffer,
...
/// <returns>"A TPCANStatus error code"</returns>
TPCANStatus __stdcall CAN_FilterMessages(TPCANHandle Channel, DWORD FromID, DWORD ToID,
                                         TPCANMode Mode);
...
/// <returns>"A TPCANStatus error code"</returns>
TPCANStatus __stdcall CAN_GetValue(TPCANHandle Channel, TPCANParameter Parameter, void* Buffer,
                                   DWORD BufferLength);
...
/// <param name="BufferLength">"Size in bytes of the buffer"</param>
/// <returns>"A TPCANStatus error code"</returns>
TPCANStatus __stdcall CAN_SetValue(TPCANHandle Channel, TPCANParameter Parameter, void* Buffer,
                                   DWORD BufferLength);
